...
4. 해당 파이프라인은 추후 서비스 mvc 와 합쳐져 하나의 모놀리식 서버로 배포된다.
5. Docker Container vs k8s
...
Consumer : AI Server
Binding Key : ai.inbound
DLX : x.retry.direct
x-dead-letter-routing-key: 2ai.retry

#### q.ai.result
Publisher : AI Server
Consumer : App Server
Binding Key : ai.result
DLX : x.retry.direct
x-dead-letter-routing-key: 2app.retry
...
x-dead-letter-routing-key: ai.inbound
binding key : 2ai.retry

#### q.2app.retry
durable: true
x-message-ttl: 30000
x-dead-letter-exchange: x.ai2app.direct
x-dead-letter-routing-key: ai.result
binding key : 2app.retry
